...
@site: 
@software: PyCharm
@file: object_size_drop.py
@time: 2021/11/19 15:49
"""
...
import imutils
import cv2
from draw import draw_histogram
...
for c in cnts:
    ...
    # print(dimA, dimB, round(dimA, 4), round(dimB, 4))
    if round(dimA, 4) not in dict_data:
        dict_data[round(dimA, 4)] = 1
    else:
        dict_data[round(dimA, 4)] = dict_data[round(dimA, 4)]+1
    ...
    # cv2.waitKey(0)
# print(dict_data)
x_data = sorted(dict_data.keys())
y_data = []
for x in x_data:
    y_data.append(dict_data[x])
# print(x_data)
# print(y_data)
# print(len(x_data))
# print(len(y_data))
draw_histogram(x_data, y_data)


# pythonmain object_size_drop.py --image target.tif --width 0.955